...
printf '@testable import SwiftUI\n'                    > "$TMP/BadTestable.swift"
printf '@preconcurrency import CoreLocation\n'         > "$TMP/BadPrecon.swift"
printf 'import Foundation  // mentions SwiftUI\n'      > "$TMP/CommentMention.swift"
printf '/* import SwiftUI in a block comment */\n'     > "$TMP/BlockComment.swift"
...
assert_exit 1 "@testable import SwiftUI rejected"     "$HERE/check-core-imports.sh" "$TMP/BadTestable.swift"
assert_exit 1 "@preconcurrency import rejected"       "$HERE/check-core-imports.sh" "$TMP/BadPrecon.swift"
assert_exit 0 "trailing // comment mention passes"    "$HERE/check-core-imports.sh" "$TMP/CommentMention.swift"
assert_exit 0 "block-comment mention passes"          "$HERE/check-core-imports.sh" "$TMP/BlockComment.swift"
...
printf '@Model final class StrParen {\n  @Attribute(originalName: "id_with_)", .unique) var id: UUID?\n}\n' > "$TMP/BadStrParen.swift"
...
assert_exit 1 "string-literal ) .unique rejected"     "$HERE/check-swiftdata-rules.sh" "$TMP/BadStrParen.swift"
